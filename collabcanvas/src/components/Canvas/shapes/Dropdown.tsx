...
import { Rect, Text as KonvaText, Line, Group } from 'react-konva';
...
/**
 * Dropdown Form Element Component
 * Visual mockup of a dropdown/select menu for wireframing
 */
const Dropdown: React.FC<DropdownProps> = ({
  ...
  const placeholder = formOptions?.placeholder || 'Select an option...';
  ...
  return (
    ...
    >
      {/* Label above dropdown if provided */}
      {label && (
        <KonvaText
          x={0}
          y={-18}
      ...
      {/* Placeholder/selected text - constrained width to prevent overflow into arrow */}
      <KonvaText
        x={10}
        y={(height - 14) / 2}
        text={placeholder}
      ...
      {/* Dropdown arrow icon (positioned at right edge) */}
      <Line
        points={[
          width - 20, height / 2 - 3,
          width - 15, height / 2 + 2,
